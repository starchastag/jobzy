...
const MyJobs = () => {
  ...
  useEffect(() => {
    fetch(`http://localhost:3000/myJobs/Shustarc786@gmail.com`).then(res => res.json()).then(data => {
      setJobs(data);
      setIsLoading(false)
    });
  }, [isLoading]);
  ...
  const currentJobs = jobs.slice(indexOfFirstItems, indexOfLastItem)
  ...
  return <div className='max-w-screen-2xl container mx-auto xl:px-24 px-4'>
    <div className='my-jobs-container'>
      ...
      <div className='search-box p-2 text-center mb-2'>
        <input onChange={(e) => setSearchText(e.target.value)} type="text" name='seatch' className='py-2 pl-3 border focus:outline-none lg:6/12 mb-4 w-ful' />
    ...
    {/* table  */}
        <section className="py-1 bg-blueGray-50">
          <div className="w-full xl:w-ful mb-1 xl:mb-0 px-4 mx-auto mt-5">
            <div className="relative flex flex-col min-w-0 break-words bg-white w-full mb-6 shadow-lg rounded ">
              ...
              <div className="block w-full overflow-x-auto">
                <table className="items-center bg-transparent w-full border-collapse ">
                  ...
                  {
                    isLoading ? (<div className='flex items-center justify-center h-20'><p>Loding...</p></div>) : (
                      <tbody>
                        {
                          currentJobs.map((job, index) => (
                            <tr key={index}>
                              ...
                              <td className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
                                <button><Link to={`/edit-job/${job?._id}`}>Update</Link></button>
                              </td>
                              <td className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
